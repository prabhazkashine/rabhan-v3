...
const project_client_1 = require("../utils/project-client");
...
class TicketService {
     ...
     */
    async createTicket(data, userId, authToken) {
        try {
            logger_1.logger.info('Creating new ticket', {
                project_id: data.project_id,
                user_id: userId
            });
            const project = await (0, project_client_1.getProjectById)(data.project_id, authToken);
            if (!project) {
                throw new error_handler_1.AppError('Project not found', 404);
            }
            if (!(0, project_client_1.isUserProjectOwner)(project, userId)) {
                throw new error_handler_1.AppError('You do not have permission to create a ticket for this project', 403);
            }
            if (!(0, project_client_1.isProjectCompleted)(project)) {
                throw new error_handler_1.AppError('Support tickets can only be created for projects with status "installation_completed" or "completed"', 400);
            }
            ...
            logger_1.logger.info('Ticket created successfully', {
                ticket_id: ticket.id,
                project_id: data.project_id,
                contractor_id: project.contractor_id
            });
            ...
            logger_1.logger.error('Error creating ticket:', {
     ...
     */
    async createAdminTicket(data, userId, userRole) {
        try {
            logger_1.logger.info('Creating new admin ticket', {
                user_id: userId,
                user_role: userRole,
                admin_id: 'unassigned'
            });
            ...
            logger_1.logger.info('Admin ticket created successfully', {
                ticket_id: ticket.id,
                user_id: userId,
                user_role: userRole
            });
            ...
            logger_1.logger.error('Error creating admin ticket:', {
                error: error instanceof Error ? error.message : 'Unknown error',
                user_id: userId,
                user_role: userRole
            });
     ...
     */
    async getAdminTickets(adminId, userRole, filters) {
        try {
            ...
            logger_1.logger.info('Retrieved admin tickets', {
            ...
            logger_1.logger.error('Error retrieving admin tickets:', {
                error: error instanceof Error ? error.message : 'Unknown error',
                admin_id: adminId,
                user_role: userRole
            });
     ...
     */
    async assignAdminTicket(ticketId, adminId, assignedById, assignedByRole) {
        try {
            const ticket = await prisma.ticket.findUnique({
                where: { id: ticketId }
            });
            if (!ticket) {
                throw new error_handler_1.AppError('Ticket not found', 404);
            }
            if (ticket.ticket_type !== 'admin_support') {
                throw new error_handler_1.AppError('Only admin support tickets can be assigned to admins', 400);
            }
            ...
            logger_1.logger.info('Admin ticket assigned', {
                ticket_id: ticketId,
                admin_id: adminId,
                assigned_by: assignedById
            });
            ...
            logger_1.logger.error('Error assigning admin ticket:', {
                error: error instanceof Error ? error.message : 'Unknown error',
                ticket_id: ticketId,
                admin_id: adminId
            });
    ...
    async getUserTickets(userId, filters) {
        try {
            ...
            if (filters?.ticket_type)
                whereClause.ticket_type = filters.ticket_type;
            const [tickets, total] = await Promise.all([
                prisma.ticket.findMany({
                    where: whereClause,
                    include: {
            ...
            logger_1.logger.info('Retrieved user tickets', {
                user_id: userId,
                count: tickets.length,
                total,
                page,
            ...
            logger_1.logger.error('Error retrieving user tickets:', {
                error: error instanceof Error ? error.message : 'Unknown error',
                user_id: userId
            });
            throw error;
     ...
     */
    async getContractorTickets(contractorId, filters) {
        try {
            const page = filters?.page || 1;
            const limit = filters?.limit || 10;
            const skip = (page - 1) * limit;
            ...
            logger_1.logger.info('Retrieved contractor tickets', {
            ...
            logger_1.logger.error('Error retrieving contractor tickets:', {
                error: error instanceof Error ? error.message : 'Unknown error',
                contractor_id: contractorId
            });
            throw error;
    ...
    async getTicketById(ticketId) {
        try {
            ...
            if (ticket) {
                logger_1.logger.debug('Retrieved ticket', {
                    ticket_id: ticketId
                });
            }
            return ticket;
            ...
            logger_1.logger.error('Error retrieving ticket:', {
                error: error instanceof Error ? error.message : 'Unknown error',
                ticket_id: ticketId
            });
            throw error;
     ...
     */
    async updateTicket(ticketId, data, userId, userRole) {
        try {
            ...
            if (userRole === 'user' && ticket.user_id !== userId) {
                throw new error_handler_1.AppError('You do not have permission to update this ticket', 403);
            }
            if (userRole === 'contractor') {
                // Contractor can update project_support tickets assigned to them
                // OR admin_support tickets they created
                const canUpdate = (ticket.ticket_type === 'project_support' && ticket.contractor_id === userId) ||
            ...
            logger_1.logger.info('Ticket updated', {
                ticket_id: ticketId,
                updated_by: userId,
     ...
     */
    async updateTicketStatus(ticketId, data, userId, userRole) {
        try {
            const ticket = await this.getTicketById(ticketId);
            if (!ticket) {
                throw new error_handler_1.AppError('Ticket not found', 404);
            ...
            if (data.status === 'resolved') {
                if (ticket.ticket_type === 'project_support') {
                    if (userRole !== 'user' || ticket.user_id !== userId) {
                        throw new error_handler_1.AppError('Only the user who created this ticket can mark it as resolved', 403);
                    }
                }
                else if (ticket.ticket_type === 'admin_support') {
                    const canResolve = (userRole === 'user' && ticket.user_id === userId) ||
                        (userRole === 'contractor' && ticket.user_id === userId) ||
                        (userRole === 'admin' && ticket.admin_id === userId) ||
                        (userRole === 'super_admin');
                    if (!canResolve) {
                        throw new error_handler_1.AppError('You do not have permission to resolve this ticket', 403);
                    }
                }
            }
            const updateData = {
            ...
            logger_1.logger.info('Ticket status updated', {
                ticket_id: ticketId,
                old_status: ticket.status,
                new_status: data.status,
                updated_by: userId,
            ...
            logger_1.logger.error('Error updating ticket status:', {
                error: error instanceof Error ? error.message : 'Unknown error',
                ticket_id: ticketId,
                user_id: userId
            });
     ...
     */
    async addReply(ticketId, data, userId, userRole) {
        try {
            const ticket = await this.getTicketById(ticketId);
            if (!ticket) {
                throw new error_handler_1.AppError('Ticket not found', 404);
            ...
            if (ticket.ticket_type === 'project_support') {
                if (!['user', 'contractor'].includes(userRole)) {
                    throw new error_handler_1.AppError('Only user or contractor can reply to project support tickets', 403);
                }
                if (userRole === 'user' && ticket.user_id !== userId) {
                    throw new error_handler_1.AppError('You do not have permission to reply to this ticket', 403);
                }
                ...
                if (!['user', 'contractor', 'admin', 'super_admin'].includes(userRole)) {
                    throw new error_handler_1.AppError('Invalid role for replying to admin support tickets', 403);
                }
                if (userRole === 'user' && ticket.user_id !== userId) {
                    throw new error_handler_1.AppError('You do not have permission to reply to this ticket', 403);
                }
                if (userRole === 'contractor' && ticket.user_id !== userId) {
                    throw new error_handler_1.AppError('You can only reply to admin tickets you created', 403);
                }
                if (userRole === 'admin' && ticket.admin_id !== userId) {
                    throw new error_handler_1.AppError('You can only reply to tickets assigned to you', 403);
                }
            }
            ...
            logger_1.logger.info('Reply added to ticket', {
                ticket_id: ticketId,
                reply_id: reply.id,
                replied_by: userId,
            ...
            logger_1.logger.error('Error adding reply to ticket:', {
                error: error instanceof Error ? error.message : 'Unknown error',
                ticket_id: ticketId,
                user_id: userId
            });
    ...
    async getTicketReplies(ticketId) {
        try {
            ...
            logger_1.logger.debug('Retrieved ticket replies', {
                ticket_id: ticketId,
            ...
            logger_1.logger.error('Error retrieving ticket replies:', {
                error: error instanceof Error ? error.message : 'Unknown error',
                ticket_id: ticketId
            });
            throw error;
     ...
     */
    async createTimelineEntry(ticketId, data) {
        try {
            ...
            logger_1.logger.debug('Timeline entry created', {
                ticket_id: ticketId,
                event_type: data.event_type
            });
            ...
            logger_1.logger.error('Error creating timeline entry:', {
                error: error instanceof Error ? error.message : 'Unknown error',
                ticket_id: ticketId
            });
            throw error;
    ...
    async getTicketTimeline(ticketId) {
        try {
            ...
            logger_1.logger.debug('Retrieved ticket timeline', {
                ticket_id: ticketId,
            ...
            logger_1.logger.error('Error retrieving ticket timeline:', {
                error: error instanceof Error ? error.message : 'Unknown error',
                ticket_id: ticketId
            });
            throw error;
     ...
     */
    async addTicketDocument(ticketId, documentData, userId, userRole) {
        try {
            ...
            logger_1.logger.info('Document added to ticket', {
                ticket_id: ticketId,
            ...
            logger_1.logger.error('Error adding document to ticket:', {
                error: error instanceof Error ? error.message : 'Unknown error',
                ticket_id: ticketId,
                user_id: userId
            });
    ...
    async getTicketDocuments(ticketId) {
        try {
            ...
            logger_1.logger.error('Error retrieving ticket documents:', {
                error: error instanceof Error ? error.message : 'Unknown error',
                ticket_id: ticketId
            });
            throw error;
